...
def randomize_perturbations(env):
    ...
    Ensure that at least one perturbation is applied; if none are applied, resample.
    """
    while True:  # Keep resampling until at least one perturbation is applied
                    ...
                    if perturbation['scenario'] == 'stuck_off_two_thrusters_same_face':
                        ...
                        for thruster in selected_thrusters:
                            if perturbation['scenario'] == 'stuck_off_truster':
                                ...
                                perturbation_log.append((thruster, start_time, f"faulty_valve ({min_valve:.2f}, {max_valve:.2f})"))
        ...
        # After applying perturbations, check if any were applied
        if perturbation_log:
            break  # At least one perturbation was applied, so break the loop
...
for i in range(start_MC, num_MC):
    ...
    print(f"MC run {i} complete ot of {num_MC} total runs.")
