...
plt.rcParams.update({
    ...
    "pgf.preamble": r"\usepackage{amsmath} \usepackage{amsfonts} \usepackage{graphicx}",  # Add other packages if needed
    "font.size": 30,                # Overall font size
...
class TrajectoryPlotter:
    ...
    def find_waypoints_path_length(self, positions):
        ...
        print(diffs)
        path_dist = np.sum(distances)

        print("Path distance [m]:", path_dist)

    def find_average_lateral_deviation(self, df):
        lateral_error_array = df['tracking_error'].to_list()
        print("Average lateral error [m]: ", np.mean(lateral_error_array))

    def find_total_impulse(self, df):
        # u_dem = df['u_demanded'].to_numpy()
        dt = 0.02  # TODO(kalbee): verify accuracy!

        u_demanded = np.vstack(df['u_demanded'].values)
        u_demanded = np.sum(u_demanded)*0.02
        print("Total impulse requested [N-s]: ", u_demanded)

    def find_final_time(self, df):
        # columns = df.columns.tolist()
        # print(columns)

        # get last datapoint
        last_row = df.tail(1)  # Get the last row
        tf = last_row['Timestamp'].values[0]
        runtime_tf = df['solve_time'].to_list()
        runtime_tf = np.mean(runtime_tf)

        print("tf: ", tf)
        print("runtime_hz: ", 1.0/runtime_tf)

    def add_trajectory_with_pos_and_tube(self, ax,
        start_point,
        end_point,
        df,
        subsample_step=1,
        ...
        # Validate input
        if start_point < 1 or end_point > len(self.positions):
        ...
        positions_subset = self.positions[start_idx:end_idx]
    ...
    def add_trajectory_with_pos(self, ax,
        start_point,
        end_point,
        df,
        subsample_step=1,
        ...
        ax.set_ylabel('Y [m]')
        ax.set_zlabel('Z [m]')
        ax.set_title('Tracking $\mathbf{x}_{ins,k}$ inside $\mathcal{X}_{in}$')
