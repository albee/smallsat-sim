...
def apply_failure_scenarios(
    ...
    *,
    ...
    thrusters_now = table_thrusters[scenario_indices]
    env_rows = jnp.broadcast_to(selected_envs[:, None], thrusters_now.shape)
    safe_envs = jnp.where(valid_now, env_rows, 0).astype(jnp.int32)
    safe_thrusters = jnp.where(valid_now, thrusters_now, 0).astype(jnp.int32)
    safe_status = jnp.where(valid_now, status_now, 0).astype(jnp.int32)
    status_updates = jnp.zeros_like(Perturbation.thruster_mask).at[
        safe_envs, safe_thrusters
    ].max(safe_status)
    Perturbation.thruster_mask = jnp.where(
        status_updates != 0, status_updates, Perturbation.thruster_mask
    )

    perturbations = env.perturbations.perturbations
    for failure_type, failure_status in SCENARIO_FAILURE_STATUS.items():
        type_mask = jnp.logical_and(valid_now, status_now == int(failure_status))
        if not bool(jnp.any(type_mask)):
            ...
            continue
        start_times = jnp.zeros_like(Perturbation.thruster_mask, dtype=jnp.float32).at[
            safe_envs, safe_thrusters
        ...
        if failure_type == 0:
            ...
            sampled_force = jax.random.uniform(
                ...
                minval=perturbation.min_thruster_force[safe_thrusters],
                maxval=perturbation.max_thruster_force[safe_thrusters],
            )
            stuck_on_mask_slots = jnp.logical_and(type_mask, valid_now)
            stuck_on_mask_matrix = jnp.zeros_like(perturbation.stuck_on_force, dtype=bool).at[
                safe_envs, safe_thrusters
            ].max(stuck_on_mask_slots)
            stuck_on_updates = jnp.zeros_like(perturbation.stuck_on_force).at[
                safe_envs, safe_thrusters
            ].max(jnp.where(stuck_on_mask_slots, sampled_force, 0.0))
            perturbation.stuck_on_force = jnp.where(
                stuck_on_mask_matrix, stuck_on_updates, perturbation.stuck_on_force
            )
            perturbation.state = PerturbationState(
                rng=perturbation._key,
                thruster_mask=Perturbation.thruster_mask,
                failure_value=failure_status,
                start_times=start_times,
                max_thruster_force=perturbation.stuck_on_force,
            )
        else:
