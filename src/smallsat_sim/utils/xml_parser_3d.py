...
def generate_mujoco_xml_flat_bed(env_config, model_config, free_floating: bool = False):
    ...
    # Import meshes of a plane and open the worldbody section
...
if __name__ == "__main__":
    ...
    from smallsat_sim.envs.smallsat_hardware_3d.cfg.config import EnvConfig
    from smallsat_sim.model.smallsat_hardware_3d.cfg.config import ModelConfig

    model_config = ModelConfig()
    env = EnvConfig()
    xml = generate_mujoco_xml(env, model_config)
    with open("smallsat_hardware_3d.xml", "w") as f:
        f.write(xml)
    print(xml)
